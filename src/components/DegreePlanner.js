...
import {
  ...
  TrendingUp, Layers, Map, Archive
} from "lucide-react";
import { computeAllStats } from "../utils/gradeUtils";
...
const STATUS_COLORS = {
  ...
  "Transferred": { bg:"rgba(20,184,166,0.12)",  text:"#2dd4bf",  border:"rgba(20,184,166,0.25)"  },
  "In Progress": { bg:"rgba(59,130,246,0.12)",  text:"#60a5fa",  border:"rgba(59,130,246,0.25)"  },
  "Not Started": { bg:"rgba(255,255,255,0.05)", text:"#64748b",  border:"rgba(255,255,255,0.1)"  },
  "Waived":      { bg:"rgba(168,85,247,0.12)",  text:"#c084fc",  border:"rgba(168,85,247,0.25)"  },
  "Archived":    { bg:"rgba(16,185,129,0.1)",   text:"#34d399",  border:"rgba(16,185,129,0.2)"   },
};
...
const TABS = [
  ...
  { key:"future",    label:"Plan Ahead", icon:Map          },
...
export default function DegreePlanner() {
  ...
  const [allClasses, setAllClasses]   = useState([]);   // active + archived
  ...
  const [degreeSettings, setDegreeSettings] = useState({
    totalCredits:120, minorName:"Minor", minorCredits:18,
    coreCategories:DEFAULT_CORE_CATS,
  });
  ...
  useEffect(() => {
    ...
    // ALL classes (active + archived) so we can show completed courses
    subs.push(onSnapshot(
      query(collection(db,"classes"), where("uid","==",uid)),
      s => setAllClasses(s.docs.map(d=>({id:d.id,...d.data()})))
    ));
    subs.push(onSnapshot(
  ...
  useEffect(() => {
    if (semesters.length && !Object.keys(expandedSems).length)
  ...
  /* coreCats from settings */
  const coreCats = degreeSettings.coreCategories || DEFAULT_CORE_CATS;

  /* ── Sources for each requirement tab ── */
  // Archived classes → show in their category with status "Completed"
  const archivedAsCourses = archivedClasses.map(cls => ({
    ...cls,
    source: "archived",
    status: cls.is_transfer ? "Transferred" : "Completed",
    credit_hours: Number(cls.credit_hours||0),
    category: cls.category || "Major",
  }));

  const getCoreForCat = catName => {
    const fromActive   = activeClasses.filter(c=>c.category==="Core"&&c.core_category===catName)
                          .map(c=>({...c,source:"current"}));
    const fromArchived = archivedAsCourses.filter(c=>c.category==="Core"&&c.core_category===catName)
                          .map(c=>({...c,source:"archived"}));
    const fromManual   = coreReqs.filter(c=>c.category===catName||c.core_category===catName)
                          .map(c=>({...c,source:"manual"}));
    const fromSems     = semesters.flatMap(s=>(s.courses||[])
                          .filter(c=>c.category==="Core"&&c.core_category===catName)
                          .map(c=>({...c,source:"semester",semName:s.name})));
    return [...fromActive,...fromArchived,...fromManual,...fromSems];
  };

  const getMajorCourses = () => {
    const fromActive   = activeClasses.filter(c=>c.category==="Major").map(c=>({...c,source:"current"}));
    const fromArchived = archivedAsCourses.filter(c=>c.category==="Major").map(c=>({...c,source:"archived"}));
    const fromManual   = majorReqs.map(c=>({...c,source:"manual"}));
    const fromSems     = semesters.flatMap(s=>(s.courses||[]).filter(c=>c.category==="Major")
                          .map(c=>({...c,source:"semester",semName:s.name})));
    return [...fromActive,...fromArchived,...fromManual,...fromSems];
  };

  const getMinorCourses = () => {
    const fromActive   = activeClasses.filter(c=>c.category==="Minor").map(c=>({...c,source:"current"}));
    const fromArchived = archivedAsCourses.filter(c=>c.category==="Minor").map(c=>({...c,source:"archived"}));
    const fromManual   = minorReqs.map(c=>({...c,source:"manual"}));
    const fromSems     = semesters.flatMap(s=>(s.courses||[]).filter(c=>c.category==="Minor")
                          .map(c=>({...c,source:"semester",semName:s.name})));
    return [...fromActive,...fromArchived,...fromManual,...fromSems];
  };
  ...
  const addSemester = async e => {
    ...
    const ref = await addDoc(collection(db,"degreeSemesters"),{
      name:semName.trim(), uid:user.uid, courses:[], order:semesters.length+1
    });
    setExpandedSems(p=>({...p,[ref.id]:true}));
  ...
  const addCourseToSemester = async e => {
    e.preventDefault();
    const sem = semesters.find(s=>s.id===courseModal);
    if (!sem) return;
    const newCourse = {...courseForm, id:Date.now().toString()};
    await updateDoc(doc(db,"degreeSemesters",courseModal),{
  ...
  const addRequirement = async e => {
    e.preventDefault();
    const col = reqModal==="core"?"coreRequirements":reqModal==="major"?"majorRequirements":"minorRequirements";
  ...
  const statusPill = (status, onChange) => {
    const m = STATUS_COLORS[status] || STATUS_COLORS["Not Started"];
    if (onChange) {
      return (
        <select className="dp-cell-select" value={status}
  ...
  const semesterCredits = sem => (sem.courses||[]).reduce((s,c)=>s+Number(c.credit_hours||0),0);
    ...
    <div className="degree-planner">
      ...
      <div className="dp-header">
        <div>
          ...
          <p>Plan and track your full academic journey</p>
        </div>
      ...
      <div className="dp-progress-hero stagger-1">
        <div className="dp-progress-stats">
          <div className="dp-pstat">
            <span className="dp-pstat-val">{stats.completedCredits}</span>
            <span className="dp-pstat-label">Completed</span>
          </div>
          <div className="dp-pstat teal">
            <span className="dp-pstat-val">{stats.transferCredits}</span>
            <span className="dp-pstat-label">Transferred</span>
          </div>
          <div className="dp-pstat blue">
            <span className="dp-pstat-val">{stats.inProgressCredits}</span>
            <span className="dp-pstat-label">In Progress</span>
          </div>
          <div className="dp-pstat amber">
            <span className="dp-pstat-val">{stats.remaining}</span>
            <span className="dp-pstat-label">Remaining</span>
          </div>
          <div className="dp-pstat purple">
            <span className="dp-pstat-val">{stats.cumulativeGpa?.toFixed(2)||"—"}</span>
            <span className="dp-pstat-label">Cumulative GPA</span>
          </div>
        </div>
        <div className="dp-progress-bar-wrap">
          ...
          <div className="dp-progress-track">
            <div className="dp-progress-fill" style={{width:`${stats.degreeProgress}%`}}/>
            {stats.inProgressCredits>0 && (
              <div className="dp-progress-inprog" style={{
                width:`${Math.min((stats.inProgressCredits/stats.degreeReq)*100, 100-stats.degreeProgress)}%`,
                left:`${stats.degreeProgress}%`
              }}/>
            )}
          </div>
          <div className="dp-progress-legend">
            <span className="dp-leg green">Completed ({stats.completedCredits} cr)</span>
            <span className="dp-leg teal">Transferred ({stats.transferCredits} cr)</span>
            <span className="dp-leg blue">In Progress ({stats.inProgressCredits} cr)</span>
          </div>
        </div>

        {/* GPA note */}
        <div className="dp-gpa-note">
          <TrendingUp size={13}/>
          Cumulative GPA <strong>{stats.cumulativeGpa?.toFixed(3)||"—"}</strong>
          {" "}computed from {stats.archGpaCr} completed non-transfer credit hours.
          Transfer credits count toward degree but not GPA.
        </div>
      </div>
          ...
          <div className="dp-overview-grid">
            ...
            {activeClasses.length>0 && (
              <div className="dp-overview-card">
                <div className="dp-ov-header"><Clock size={15}/><span>Current Semester</span>
                  <span className="dp-ov-cr">{activeClasses.reduce((s,c)=>s+Number(c.credit_hours||0),0)} cr</span>
                </div>
            ...
            {/* Archived/completed classes */}
            {archivedClasses.length>0 && (
              <div className="dp-overview-card">
                <div className="dp-ov-header"><Archive size={15}/><span>Completed Classes</span>
                  <span className="dp-ov-cr">{archivedClasses.reduce((s,c)=>s+Number(c.credit_hours||0),0)} cr</span>
                </div>
                <div className="dp-course-table">
                  {archivedClasses.map(cls=>(
                    <div key={cls.id} className="dp-course-row">
                      <span className="dp-cr-code">{cls.course_code}</span>
                      <span className="dp-cr-name">{cls.course_name}</span>
                      <span className="dp-cr-hrs">{cls.credit_hours}cr</span>
                      {cls.final_letter && <span className="dp-final-badge">{cls.final_letter}</span>}
                      {statusPill(cls.is_transfer?"Transferred":"Completed")}
                    </div>
                  ))}
                </div>
              </div>
      ...
      {activeTab==="semesters" && (
        <div className="dp-tab-content stagger-3">
          <div className="dp-section-actions">
            <h3 className="dp-section-title">Semester Plans</h3>
          ...
          {activeClasses.length>0 && (
            <div className="dp-semester-block current-sem">
              <div className="dp-sem-header">
                ...
                <span className="dp-sem-cr">{activeClasses.reduce((s,c)=>s+Number(c.credit_hours||0),0)} credits</span>
              </div>
              <div className="dp-course-table dp-table-full">
                <div className="dp-table-head"><span>Code</span><span>Name</span><span>Cr</span><span>Category</span><span>Status</span></div>
                {activeClasses.map(cls=>(
                  <div key={cls.id} className="dp-course-row">
                    <span className="dp-cr-code" style={{color:cls.color}}>{cls.course_code}</span>
                    <span className="dp-cr-name">{cls.course_name}</span>
          ...
          {/* Archived classes grouped by semester */}
          {Object.entries(
            archivedClasses.reduce((acc,cls)=>{
              const key = cls.semester||"Unsorted";
              if(!acc[key]) acc[key]=[];
              acc[key].push(cls); return acc;
            },{})
          ).map(([semName, clsList])=>(
            <div key={semName} className="dp-semester-block">
              <div className="dp-sem-header">
                <div className="dp-sem-left">
                  <span className="dp-sem-badge archived-sem-badge">ARCHIVED</span>
                  <span className="dp-sem-name">{semName}</span>
                </div>
                <span className="dp-sem-cr">
                  {clsList.reduce((s,c)=>s+Number(c.credit_hours||0),0)} credits
                </span>
              </div>
              <div className="dp-course-table dp-table-full">
                <div className="dp-table-head"><span>Code</span><span>Name</span><span>Cr</span><span>Grade</span><span>Status</span></div>
                {clsList.map(cls=>(
                  <div key={cls.id} className="dp-course-row row-done">
                    <span className="dp-cr-code">{cls.course_code}</span>
                    <span className="dp-cr-name">{cls.course_name}</span>
                    <span className="dp-cr-hrs">{cls.credit_hours}</span>
                    <span className="dp-final-badge">
                      {cls.final_letter||"—"}{cls.final_grade_pct?` (${cls.final_grade_pct}%)`:""}</span>
                    {statusPill(cls.is_transfer?"Transferred":"Completed")}
                  </div>
                ))}
              </div>
          ...
          {semesters.map(sem=>{
            ...
            return (
              <div key={sem.id} className="dp-semester-block">
                <button className="dp-sem-header dp-sem-toggle"
                  onClick={()=>setExpandedSems(p=>({...p,[sem.id]:!p[sem.id]}))}>
                  <div className="dp-sem-left">
                    <span className="dp-sem-name">{sem.name}</span>
                    {total>0 && <span className="dp-sem-progress-mini">{done}/{total} done</span>}
                  </div>
                  <div className="dp-sem-right">
                    <span className="dp-sem-cr">{cr} cr</span>
                    <button className="btn-icon dp-del-sem"
                ...
                {isOpen && (
                  <div className="dp-sem-body">
                    ...
                    <button className="dp-add-course-btn" onClick={()=>setCourseModal(sem.id)}>
          ...
          {semesters.length===0&&activeClasses.length===0&&archivedClasses.length===0&&(
            <div className="empty-state"><div className="empty-state-icon">📅</div>
              <h3>No semesters planned</h3><p>Add a semester to start planning</p></div>
          )}
        </div>
      )}
      ...
      {activeTab==="future" && (
        <div className="dp-tab-content stagger-3">
          <div className="dp-section-actions">
            ...
            <p className="dp-section-sub">Courses marked "Not Started" or "In Progress" across planned semesters</p>
          </div>
          {futureCourses.length===0?(
            <div className="empty-state"><div className="empty-state-icon">🗺️</div>
              <h3>No future courses planned</h3></div>
          ):(
            <div className="dp-future-grid">
              {Object.entries(futureCourses.reduce((acc,c)=>{
                if(!acc[c.semName]) acc[c.semName]=[];
                acc[c.semName].push(c); return acc;
              },{})).map(([sn,courses])=>(
                <div key={sn} className="dp-future-group">
                  <div className="dp-future-sem-label">
                    <Layers size={13}/><span>{sn}</span>
                    <span className="dp-future-cr">{courses.reduce((s,c)=>s+Number(c.credit_hours||0),0)} cr</span>
                  </div>
                  {courses.map(c=>(
          ...
          <div className="dp-core-grid">
            {coreCats.map(cat=>{
              ...
              const earned  = courses.filter(c=>
                c.source==="current"||c.status==="Completed"||c.status==="Transferred"||c.source==="archived"
              ...
              return (
                <div key={cat.id} className={"dp-core-cat"+(done?" cat-done":"")}>
                  ...
                  {courses.length>0?(
                    <div className="dp-core-courses">
                      {courses.map((c,i)=>(
                        <div key={i} className="dp-core-course-row">
                          ...
                          <span className="dp-cr-name">{c.course_name}</span>
                          <span className="dp-cr-hrs">{c.credit_hours}cr</span>
                          {c.source==="current"  ? statusPill("In Progress")
                          :c.source==="archived"  ? statusPill(c.is_transfer?"Transferred":"Completed")
                          :c.source==="semester"  ? <span className="dp-sem-tag">{c.semName}</span>
                          :(
                            <>
                              <select className="dp-cell-select" value={c.status||"Not Started"}
                                onChange={e=>updateReq("core",c.id,"status",e.target.value)}
                                style={{color:STATUS_COLORS[c.status]?.text||"inherit"}}>
                                {STATUS_OPTIONS.map(s=><option key={s}>{s}</option>)}
                              </select>
                              <button className="btn-icon dp-del-course"
                                onClick={()=>setDeleteTarget({type:"core",id:c.id})}><Trash2 size={12}/></button>
                            </>
                          )}
                        </div>
                      ))}
                    ...
                    <p className="dp-empty-hint">No courses yet</p>
                  )}
                </div>
              );
            })}
      ...
      {activeTab==="major" && (
        <div className="dp-tab-content stagger-3">
          ...
          {getMajorCourses().length===0?(
            <div className="empty-state"><div className="empty-state-icon">📖</div><h3>No major courses yet</h3></div>
          ):(
            <div className="dp-req-table">
              <div className="dp-table-head"><span>Code</span><span>Name</span><span>Cr</span><span>Status</span><span></span></div>
              {getMajorCourses().map((c,i)=>(
                <div key={i} className={"dp-course-row"+(c.status==="Completed"||c.source==="archived"?" row-done":"")}>
                  <span className="dp-cr-code">{c.course_code}</span>
                  <span className="dp-cr-name">{c.course_name}</span>
                  <span className="dp-cr-hrs">{c.credit_hours}cr</span>
                  {c.source==="current"  ? statusPill("In Progress")
                  ...
                  :(
                    <>
                      ...
                      <button className="btn-icon dp-del-course"
                        onClick={()=>setDeleteTarget({type:"major",id:c.id})}><Trash2 size={12}/></button>
                    </>
                  )}
                </div>
              ))}
        ...
        <div className="dp-tab-content stagger-3">
          ...
          <div className="dp-minor-progress">
            <div className="dp-minor-prog-row">
              <span>Progress</span>
              <span>
                {getMinorCourses().filter(c=>c.status==="Completed"||c.source==="archived"||c.is_completed)
                  .reduce((s,c)=>s+Number(c.credit_hours||0),0)}
                /{degreeSettings.minorCredits||18} cr
              </span>
            </div>
            <div className="dp-mini-progress">
          ...
          {getMinorCourses().length===0?(
            <div className="empty-state"><div className="empty-state-icon">📋</div><h3>No minor courses yet</h3></div>
          ):(
            <div className="dp-req-table">
              <div className="dp-table-head"><span>Code</span><span>Name</span><span>Cr</span><span>Status</span><span></span></div>
                ...
                <div key={i} className={"dp-course-row"+(c.status==="Completed"||c.source==="archived"?" row-done":"")}>
                  ...
                  :(
                    <>
                      ...
                      <button className="btn-icon dp-del-course"
                        onClick={()=>setDeleteTarget({type:"minor",id:c.id})}><Trash2 size={12}/></button>
                    </>
                  )}
                </div>
              ))}
      ...
      {semModal&&(
        <div className="modal-overlay" onClick={()=>setSemModal(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><h2>Add Semester</h2>
              <button className="btn-icon" onClick={()=>setSemModal(false)}>✕</button></div>
            <form onSubmit={addSemester}>
              <div className="modal-body">
                <div className="form-group"><label>Semester Name *</label>
                  <input className="form-control" placeholder="e.g. Fall 2025"
                    value={semName} onChange={e=>setSemName(e.target.value)} required autoFocus/></div>
              </div>
      ...
      {courseModal&&(
        <div className="modal-overlay" onClick={()=>setCourseModal(null)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><h2>Add Course</h2>
              <button className="btn-icon" onClick={()=>setCourseModal(null)}>✕</button></div>
            <form onSubmit={addCourseToSemester}>
              <div className="modal-body dp-course-form">
                <div className="form-row">
                  <div className="form-group"><label>Course Code *</label>
                    <input className="form-control" placeholder="CS 3345"
                      ...
                      onChange={e=>setCourseForm(f=>({...f,course_code:e.target.value}))} required/></div>
                  <div className="form-group"><label>Credit Hours</label>
                    <input type="number" min="0" max="12" className="form-control"
                      value={courseForm.credit_hours}
                      onChange={e=>setCourseForm(f=>({...f,credit_hours:Number(e.target.value)}))}/></div>
                </div>
                <div className="form-group"><label>Course Name *</label>
                  <input className="form-control" placeholder="Data Structures"
                    value={courseForm.course_name}
                    onChange={e=>setCourseForm(f=>({...f,course_name:e.target.value}))} required/></div>
                <div className="form-row">
                  <div className="form-group"><label>Category</label>
                    <select className="form-control" value={courseForm.category}
                      onChange={e=>setCourseForm(f=>({...f,category:e.target.value}))}>
                      {CATEGORY_OPTIONS.map(c=><option key={c}>{c}</option>)}</select></div>
                ...
                {courseForm.category==="Core"&&(
                  <div className="form-group"><label>Core Category</label>
          ...
          <div className="modal" onClick={e=>e.stopPropagation()}>
            ...
            <form onSubmit={addRequirement}>
              <div className="modal-body dp-course-form">
                <div className="form-row">
                  <div className="form-group"><label>Course Code *</label>
                    <input className="form-control" value={reqForm.course_code}
                      onChange={e=>setReqForm(f=>({...f,course_code:e.target.value}))} required/></div>
                  <div className="form-group"><label>Credits</label>
                    <input type="number" min="0" className="form-control" value={reqForm.credit_hours}
                      onChange={e=>setReqForm(f=>({...f,credit_hours:Number(e.target.value)}))}/></div>
                </div>
                <div className="form-group"><label>Course Name *</label>
                  <input className="form-control" value={reqForm.course_name}
                    onChange={e=>setReqForm(f=>({...f,course_name:e.target.value}))} required/></div>
                {reqModal==="core"&&(
                  <div className="form-group"><label>Core Category *</label>
      ...
      {settingsOpen&&(
        <div className="modal-overlay" onClick={()=>setSettingsOpen(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()}>
            <div className="modal-header"><h2>Degree Settings</h2>
              <button className="btn-icon" onClick={()=>setSettingsOpen(false)}>✕</button></div>
            <div className="modal-body dp-settings-form">
              <div className="form-row">
                <div className="form-group"><label>Total Credits Required</label>
                  <input type="number" className="form-control" value={degreeSettings.totalCredits||120}
                    onChange={e=>saveSettings({totalCredits:Number(e.target.value)})}/></div>
                <div className="form-group"><label>Minor Credits Required</label>
                  <input type="number" className="form-control" value={degreeSettings.minorCredits||18}
                    onChange={e=>saveSettings({minorCredits:Number(e.target.value)})}/></div>
              </div>
              <div className="form-group"><label>Minor Name</label>
                <input className="form-control" value={degreeSettings.minorName||""}
                  onChange={e=>saveSettings({minorName:e.target.value})}/></div>
              <div className="dp-settings-note">Changes save automatically.</div>
            </div>
            <div className="modal-footer">
